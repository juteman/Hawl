...
public:
    ...
    {
        for (INT32 shareRefCntRecord = m_shareRefCnt.load(std::memory_order_relaxed);
             shareRefCntRecord != 0;
...
public:
    ...
    RefCntDeleter(ValueType value, DeleterType deleter)
        : m_value{value}, m_deleter{std::move(deleter)}
    {
    }
    ...
    void DestroyRefCnt() noexcept override
    {
        this->~RefCntDeleter();
    }

    // ReSharper disable once CppCStyleCast
    void *GetDeleter() const noexcept
    {
