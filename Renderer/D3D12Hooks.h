...
extern HRESULT hook_create_device(void *pAdapter,
                                  D3D_FEATURE_LEVEL featureLevel,
                                  ID3D12Device **ppDevice);
extern HRESULT hook_create_command_queue(ID3D12Device *pDevice,
                                         const D3D12_COMMAND_QUEUE_DESC *qDesc,
                                         ID3D12CommandQueue **ppQueue);
extern HRESULT hook_create_copy_cmd(ID3D12Device *pDevice,
                                    uint32_t nodeMask,
                                    ID3D12CommandAllocator *pAlloc,
                                    Cmd *pCmd);
extern void hook_remove_copy_cmd(Cmd *pCmd);
extern HRESULT hook_create_graphics_pipeline_state(ID3D12Device *pDevice,
                                                   const D3D12_GRAPHICS_PIPELINE_STATE_DESC *pDesc,
                                                   void *pExtensions,
                                                   uint32_t extensionCout,
                                                   ID3D12PipelineState **ppPipeline);
extern HRESULT hook_create_compute_pipeline_state(ID3D12Device *pDevice,
...
extern HRESULT       hook_queue_present(Queue *    pQueue,
                                        SwapChain *pSwapChain,
                                        uint32_t   swapChainImageIndex);
extern void hook_dispatch(Cmd *    pCmd,
                          uint32_t groupCountX,
                          uint32_t groupCountY,
                          uint32_t groupCountZ);
extern void hook_signal(Queue *pQueue, ID3D12Fence *pDxFence, uint64_t fenceValue);

extern void hook_fill_gpu_desc(Renderer *        pRenderer,
                               D3D_FEATURE_LEVEL featureLevel,
                               GpuDesc *         pInOutDesc);
...
extern void hook_modify_buffer_resource_desc(const BufferDesc *   pBuffer,
                                             D3D12_RESOURCE_DESC *pInOutDesc);
extern void hook_modify_texture_resource_flags(TextureCreationFlags  creationFlags,
                                               D3D12_RESOURCE_FLAGS *pInOutFlags);
extern void hook_modify_shader_compile_flags(uint32_t      stage,
                                             bool          enablePrimitiveId,
                                             const WCHAR **pData,
                                             uint32_t *    pInOutNumFlags);
extern void hook_modify_rootsignature_flags(uint32_t                    stages,
                                            D3D12_ROOT_SIGNATURE_FLAGS *pInOutFlags);
extern void hook_modify_command_signature_desc(D3D12_COMMAND_SIGNATURE_DESC *pInOutDesc,
                                               uint32_t                      padding);
